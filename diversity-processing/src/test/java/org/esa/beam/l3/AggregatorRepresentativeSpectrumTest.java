...
import static org.esa.beam.l3.AggregatorRepresentativeSpectrum.computeSpectralAngle;
...
public class AggregatorRepresentativeSpectrumTest {
    ...
    public void testComputeSpectralAngle() throws Exception {
        ...
        double spectralAngle = computeSpectralAngle(refSpectrum, medianSpectrum);
        assertEquals(0.11851214679718568, spectralAngle, 1E-5);

        medianSpectrum = new double[]{1, 3, 7};
        spectralAngle = computeSpectralAngle(refSpectrum, medianSpectrum);
        assertEquals(0.2513167064860358, spectralAngle, 1E-5);
    }

    @Test
    public void testMetadata() {
        Aggregator agg = new AggregatorRepresentativeSpectrum(varCtx, "r1", "r2", "r3");
