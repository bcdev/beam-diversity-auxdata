...
# reprocessing lakes with non-default r490, 20171205: 
# regions = []
#regionsFile1 = open("./wkt/lakes-WB-repro-regions.txt","r")
#for line in regionsFile1:
#    name = line.strip().replace('\'', '')
#    regions.append(name)

regions = ['Lake-TEHUANTEPEC']
...
hosts = [('localhost', 16)]
#   , simulation=True
pm = PMonitor(inputs, request='lake_products_WB', logdir='log', hosts=hosts, script='template.py')
...
for region in regions:
    ...
    for year in years:
        ...
        for month in getMonth(year):
            ...
            pm.execute('l3-monthly_WB.xml', all_l2_names, [l3_month_name], parameters=params, logprefix=l3_month_name)
        ...
        if year != '2002' and year != '2012':
            ...
            pm.execute('l3-from-l3_WB.xml', l3_month_names, [l3_year_name], parameters=params, logprefix=l3_year_name)
    ...
    if l3_year_names:
        ...
        pm.execute('l3-from-l3_WB.xml', l3_year_names, [l3_decade_name], parameters=params, logprefix=l3_decade_name)
